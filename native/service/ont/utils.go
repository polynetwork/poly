...
func AddLockNotifications(native *native.NativeService, contract common.Address, toChainID uint64, from common.Address, to []byte, value uint64) {
	if !config.DefConfig.Common.EnableEventLog {
		return
	}
	native.AddNotify(
		&event.NotifyEventInfo{
			ContractAddress: contract,
			States:          []interface{}{LOCK_NAME, toChainID, from, to, value},
		})
}

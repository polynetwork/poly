...
func (this *Args) Deserialization(source *common.ZeroCopySource) error {
	...
	if eof {
		return fmt.Errorf("Args.Deserialization error: %s", "decode ToAddress var bytes error")
	}

	this.Value, eof = source.NextVarUint()
	if eof {
		return fmt.Errorf("Args.Deserialization error: %s", "decode Value uint64 error")
	}

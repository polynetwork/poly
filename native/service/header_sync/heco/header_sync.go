...
func needFix(native *native.NativeService) bool {
	return (config.NETWORK_ID_TEST_NET == config.DefConfig.P2PNode.NetworkId && native.GetHeight() < 14939298) || (config.NETWORK_ID_MAIN_NET == config.DefConfig.P2PNode.NetworkId && native.GetHeight() <= 12538725)
}

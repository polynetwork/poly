...
const (
	ChainID      uint64 = 603
	TestnetRpc          = "https://http-mainnet.hoosmartchain.com"
	BlocksToWait        = 21
	ECCD                = "0xA659C107970Fe62Acb5fCA2D513089e761a6CCFC" // hsc chain  ECCD contract
)
...
func NewNative(args []byte, tx *types.Transaction, db *storage.CacheDB) *native.NativeService {
	...
	contaractAddr, _ := hex.DecodeString(ccmcom.Replace0x(ECCD))
	side := &side_chain_manager.SideChain{
		Name:         "hsc",
		ChainId:      ChainID,
		BlocksToWait: BlocksToWait,
		Router:       utils.HSC_ROUTER,
...
func (this *chainClient) GetProofFromAchieveNode(contractAddress string, key string, blockheight string) ([]byte, error) {
	req := &proofReq{
		JsonRPC: "2.0",
		Method:  "eth_getProof",
...
func (this *chainClient) GetProof(contractAddress string, key string, blockheight string) ([]byte, error) {
	result := make([]byte, 0)
...
func getGenesisHeader(t *testing.T) *hsc.GenesisHeader {
	c := newChainClient()
	...
	assert.NilError(t, err)
	pvalidators, err := hsc.ParseValidators(phdr.Extra[32 : len(phdr.Extra)-65])
	assert.NilError(t, err)

	genesisHeader := hsc.GenesisHeader{Header: *hdr, PrevValidators: []hsc.HeightAndValidators{
...
func getLatestHeight(native *native.NativeService) (height uint64) {
	heightStore, err := native.GetCacheDB().Get(
		utils.ConcatKey(utils.HeaderSyncContractAddress, []byte(synccom.CURRENT_HEADER_HEIGHT), utils.GetUint64Bytes(ChainID)))
	if err != nil {
...
func TestProofHandle(t *testing.T) {
	var (
		...
		syncHandler *hsc.Handler
		handler     *HscHandler
		height      uint64
	)
	syncHandler = hsc.NewHscHandler()
	...
	var dparam *ccmcom.EntranceParam
	{
		...
		proof, err := client.GetProof(ECCD, proofKey, proofHeightHex)
		//proof, err := client.GetProofFromAchieveNode(HscCCDC, proofKey, proofHeightHex)
